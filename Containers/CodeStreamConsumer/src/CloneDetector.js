...
class CloneDetector {
  ...
  #filterLines(file) {
    ...
    // Store the number of lines in the file object
  ...
  #filterCloneCandidates(file, compareFile) {
    ...
    file.chunks.forEach((chunk, i) => {
      // Find matching chunks in the compareFile
      let matchingChunks = compareFile.chunks.filter((compareChunk, j) => {
        return this.#chunkMatch(chunk, compareChunk);
      });

      // Create Clone objects for each matching chunk
      let newInstances = matchingChunks.map((compareChunk, j) => {
        return new Clone(file.name, compareFile.name, chunk, compareChunk);
      });

      // Append new instances to file.instances
  ...
  #expandCloneCandidates(file) {
    ...
    // Use reduce to accumulate expanded clones
    file.instances = file.instances.reduce((accumulator, currentClone) => {
      // Try to expand the current clone with each clone in the accumulator
      let expanded = false;
      ...
      // If the current clone could not be expanded, add it to the accumulator
      if (!expanded) {
  ...
  #consolidateClones(file) {
    ...
    // Use reduce to accumulate unique clones
    file.instances = file.instances.reduce((accumulator, currentClone) => {
      // Find if the current clone already exists in the accumulator
      let existingClone = accumulator.find((clone) =>
        clone.equals(currentClone)
      );

      if (existingClone) {
        // If it exists, update the existing clone to include this one too
        existingClone.addTarget(currentClone);
      } else {
        // If it is new, push it into the accumulator
        accumulator.push(currentClone);
      }
